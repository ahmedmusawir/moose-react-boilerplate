...
describe('<UsersTable />', () => {
  ...
  const users = [
    ...
    },
    {
      ...
      address: '1234 Breakpoint Street',
      role: 'admin',
    },
  ];
  ...
  it('should not call deleteUser on successful init', () => {
    shallow(<UsersTable users={users} deleteUser={deleteUserSpy} />);
    expect(deleteUserSpy).not.toHaveBeenCalled();
  });

  it('should render the loading indicator when its loading', () => {
    const wrapper = shallow(<UsersTable loading deleteUser={deleteUserSpy} />);
  ...
  it('should render errors, and no users', () => {
    const wrapper = shallow(<UsersTable error deleteUser={deleteUserSpy} />);
  ...
  it('should render with UserRow and pass correct props', () => {
    ...
    expect(wrapper.contains(<UserRow {...userRowProps} />)).toBe(true);
    expect(deleteUserSpy).not.toHaveBeenCalled();
  });
});
